...
class DFA(FiniteAutomaton):
    ...
    def transitions(self):
        """Return the transitions of this DFA."""
        # tuples of the form (fromstate, tostate, onchar)
        transitions = []
        for state in self.states():
            for c in self.alphabet:
                for neighbor in self.transition(state,c):
                    transitions.append((state, neighbor, c))
        return transitions

    def to_dot(self, filename=None, render=True, view=False):
        """Render the automaton in graphviz as a PNG"""
        assert render or view, 'Please choose to either render as a PDF or view directly.'
        assert filename is not None and type(filename) == str and len(filename) > 0, 'Please provide a valid filename'
        ...
        for state in self.states():
            for c in self.alphabet:
                for neighbor in self.transition(state,c):
                    dot.edge(str(state), str(neighbor), label=c)
    ...
    def pprint(self, _):
        ...
        for state in self.states():
            ...
            if not adjectives:
                ...
                print(state, "(" + ", ".join(adjectives) + ")")
            for c in self.alphabet:
                for neighbor in self.transition(state,c):
                    print("  --[" + str(c) + "]-->", neighbor)
